...
# First value is number of accessory COG X genes, second is number of non-accessory COG X genes
COG_counts = {
    "A": [0,0],
    "B": [0,0],
    "C": [0,0],
    ...
    if category == '-':
        ...
        for line in rep_lines: 
            if line.startswith('>'):
                ...
                seq_id = get_id(line)
                if seq_id == patric_id:
                    if gene_class[cluster_count] == 'accessory':
                        if len(category) > 1:
...
for key, value in COG_counts.items():
    column_names = ['accessory Genes', 'Non-accessory Genes']
    ...
    D = (numb_genes - accessory_count) - B
...
df_odds_ratio.to_csv(species+"/"+species+"_Accessory_COG_LORs.csv", index=True)
df_p_values.to_csv(species+"/"+species+"_Accessory_COG_pvals.csv", index=True)
