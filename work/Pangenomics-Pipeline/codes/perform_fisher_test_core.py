...
# First value is number of core COG X genes, second is number of non-core COG X genes
COG_counts = {
    "C": [0,0],
    "D": [0,0],
    "E": [0,0],
    ...
    if category not in COG_counts:
        ...
        for line in rep_lines: 
            if line.startswith('>'):
                ...
                seq_id = get_id(line)
                if seq_id == patric_id:
                    if gene_class[cluster_count] == 'core':
                        if len(category) > 1:
...
for key, value in COG_counts.items():
    column_names = ['core Genes', 'Non-core Genes']
...
df_odds_ratio.to_csv(species+"/"+species+"_core_COG_LORs.csv", index=True)
df_p_values.to_csv(species+"/"+species+"_core_COG_pvals.csv", index=True)
